...
class TelaSaque(Tela):
    def __init__(self, navegador):
        super().__init__(largura=800, altura=600, titulo="Saque - Banco", cor_fundo=Cores.preto(), navegador=navegador)
        self.banco_dados = SistemaBancario()

        self.titulo = TextoFormatado(400, 100, texto="REALIZAR SAQUE", tamanho=36, centralizado=True, cor_texto=Cores.dourado())
        ...
        self.botao_sacar = Botao(300, 260, 200, 50, texto="Sacar", funcao=self.fazer_saque,
                                 cor_fundo=Cores.verde_escuro(), cor_hover=Cores.verde_menta(), cor_texto=Cores.preto(), tamanho_fonte=20)
        self.adicionar_componente(self.botao_sacar)
    ...
    def fazer_saque(self):
        try:
            ...
            if self.banco_dados.realizar_saque(self.banco_dados.id_conta_atual, valor):
                self.msg_status.atualizar_texto(f"Saque de R$ {valor:.2f} realizado com sucesso!")
            else:
                self.msg_status.atualizar_texto("Saldo insuficiente ou erro ao realizar saque!")
        except ValueError:
