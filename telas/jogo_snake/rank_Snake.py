...
class TelaRankSnake(Tela):
    def __init__(self, navegador=None):
        super().__init__(
            largura=600, altura=400, titulo="Ranking Snake",
            cor_fundo=Cores.preto(), navegador=navegador
        )
        self.navegador = navegador
        self.adicionar_componente(TextoFormatado(
            x=300, y=40, texto="RANKING SNAKE", tamanho=38, cor_texto=Cores.verde(),
    ...
    def carregar_ranking(self):
        ...
        # Filtra só cadastrados e ordena
        ranking_ordenado = sorted(
            [(apelido, pontos) for apelido, (id_usuario, pontos) in melhores.items() if id_usuario is not None],
